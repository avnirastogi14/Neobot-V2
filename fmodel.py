...
try:
    ...
    try:
        ...
        def dummy_classifier(text, candidate_labels):
            return {"labels": ["assign_role", "create_team", "update_team"], "scores": [0.9, 0.1, 0.05]} # Include update_team
        def dummy_ner(text):
            return []
...
def extract_entities(ner_results, text):
    ...
    team_patterns = [
        r"(?i)team\s+([A-Za-z][a-zA-Z0-9]*)",      # Team X, Team Alpha
        r"(?i)in\s+([A-Za-z][a-zA-Z0-9]*)\s+team",   # in Alpha team
        r"(?i)for\s+team\s+([A-Za-z][a-zA-Z0-9]*)"   # for team Alpha
    ]

    for pattern in team_patterns:
        team_match = re.search(pattern, text)
        if team_match:
            entities["team_name"] = team_match.group(1)
            break
...
def predict(user_input):
    ...
    try:
        ...
        create_team_match = re.search(r"(?i)(create|make|new)\s+(?:new\s+)?team(?:,\s+)?([A-Za-z0-9]+)", cleaned_input)
        if create_team_match:
            return {
                "intent": "create_team",
                "entities": {"team_name": create_team_match.group(2)},
                "confidence": "high",
                "processing_time": time.time() - start_time
            }
        ...
        make_role_match = re.search(r"(?i)make\s+(\w+)\s+([a-zA-Z\s]+)\s+in\s+([A-Za-z0-9]+)", cleaned_input)
        if make_role_match:
            return {
                "intent": "assign_role",
                "entities": {
        ...
        assign_to_match = re.search(r"(?i)assign\s+(\w+)\s+([a-zA-Z\s]+)\s+to\s+([A-Za-z0-9]+)", cleaned_input)
        if assign_to_match:
            return {
                "intent": "assign_role",
                "entities": {
        ...
        set_role_match = re.search(r"(?i)set\s+role\s+of\s+(\w+)\s+as\s+([a-zA-Z\s]+)\s+in\s+([A-Za-z0-9]+)", cleaned_input)
        if set_role_match:
            return {
                "intent": "assign_role",
                "entities": {
        ...
        update_repo_match = re.search(r"(?i)update\s+repo\s+of\s+([A-Za-z0-9]+)\s+to\s+(\S+)", cleaned_input)
        ...
        update_field_match = re.search(r"(?i)update\s+(role|repo|status)\s+of\s+([A-Za-z0-9]+)\s+to\s+(\S+)", cleaned_input)
